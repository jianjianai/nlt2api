...
const SUPPORTED_PROTOCOLS = new Map<string, ProxyKind>([
  ["http:", "http"],
  ["https:", "http"],
  ["socks:", "socks5"],
  ["socks4:", "socks4"],
...
function parseProxy(raw: string): ParsedProxy {
  ...
  if (!value) throw invalidProxy("`proxy` must be a non-empty string.");
  if (value.length > MAX_PROXY_URL_LENGTH) throw invalidProxy("`proxy` is too long.");
  ...
  if (!kind) throw invalidProxy("`proxy` protocol must be http, https, socks4 or socks5.");
  if (!url.hostname) throw invalidProxy("`proxy` must include a proxy host.");
...
function shorthandHostPort(value: string): { host: string; port: string; username?: string; password?: string } {
  if (value.startsWith("[")) {
    ...
    return {
      host: `[${match[1]}]`,
      port: match[2]!,
      ...(match[3] !== undefined ? { username: match[3], password: match[4] ?? "" } : {}),
    };
  }

  const hostFirst = /^([^:@]+):(\d+):([^:]*):(.*)$/.exec(value);
  if (hostFirst) {
    return { host: hostFirst[1]!, port: hostFirst[2]!, username: hostFirst[3]!, password: hostFirst[4]! };
  }
...
/** Render a proxy URL for the admin console without leaking credentials. */
...
/**
 * Strips credentials out of arbitrary text before it is stored or logged. The
 * minter reports free-form failure messages that may quote a full proxy URL.
 */
...
/**
 * Chrome accepts no credentials inside --proxy-server, so HTTP proxy auth is
 * answered over CDP (Fetch.authRequired). Chrome has no equivalent path for
 * SOCKS auth at all, which is why an authenticated SOCKS proxy can carry
 * forwarded traffic but can never mint a ticket.
 */
export function browserProxyTarget(raw: string): BrowserProxyTarget | undefined {
  ...
  const scheme = kind === "http" ? "http" : kind === "socks4" ? "socks4" : "socks5";
  const port = url.port || (kind === "http" ? "80" : "1080");
  return {
    server: `${scheme}://${url.hostname}:${port}`,
    ...(username ? { username } : {}),
    ...(username && password ? { password } : {}),
  };
}

export function isMintableProxy(raw: string): boolean {
  try {
    return browserProxyTarget(raw) !== undefined;
  } catch {
    return false;
  }
}
...
function createDispatcher(raw: string): Dispatcher {
  const { url, kind } = parseProxy(raw);
  if (kind === "http") return new ProxyAgent(url.toString());

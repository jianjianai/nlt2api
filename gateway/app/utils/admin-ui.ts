...
export const SETTING_LABEL: Record<SettingKey, { label: string; hint: string }> = {
  ...
  affinityWaitSeconds: { label: "黏滞等待（秒）", hint: "本对话的出口没票时等候专属铸票多久；超时后接受其他出口。" },
  rateLimitCooldownSeconds: { label: "429 冷却（秒）", hint: "出口遭 429 后暂停使用的时长；上游给了 Retry-After 则以它为准。" },
  ipBlockCooldownSeconds: { label: "403 冷却（秒）", hint: "出口被上游直接拒绝（403/401）后的暂停时长，应明显长于 429。" },

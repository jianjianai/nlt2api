...
const operationalItems: Array<{ id: Exclude<WorkspaceId, "settings">; label: string; icon: AppIconName }> = [
  { id: "overview", label: "运行概览", icon: "layout-dashboard" },
  ...
  <div class="workspace-shell">
    <header class="workspace-topbar">
      <div class="workspace-brand">
        ...
        <span class="workspace-brand-name">转发服务</span>
      </div>
      <div class="workspace-top-actions">
        <AppTooltip text="打开快速操作">
          <button ref="commandTrigger" class="command-trigger" type="button" aria-label="打开快速操作" @click="setCommandOpen(true)">
            <AppIcon name="search" :size="15" /><span>快速操作</span>
          </button>
        </AppTooltip>
        ...
        <label class="workspace-auto-refresh">
          <AppSwitch :model-value="autoRefresh" label="自动刷新" @update:model-value="emit('toggleAutoRefresh')" /><span>自动刷新</span>
        </label>
        <div class="theme-picker" role="group" aria-label="主题">
          <AppTooltip v-for="option in themeOptions" :key="option.id" :text="`${option.label}主题`">
            <button type="button" :class="{ active: theme === option.id }" :aria-pressed="theme === option.id" :aria-label="`切换到${option.label}主题`" @click="emit('changeTheme', option.id)">
              <AppIcon :name="option.icon" :size="14" />
            </button>
          </AppTooltip>
        </div>
        <AppTooltip :text="`当前${currentThemeOption.label}主题，点击切换`">
          <button class="icon-action mobile-theme" type="button" :aria-label="`切换主题，当前${currentThemeOption.label}主题`" @click="cycleTheme">
            <AppIcon :name="currentThemeOption.icon" :size="16" />
          </button>
        </AppTooltip>
        <AppTooltip text="运行参数">
          <button class="icon-action mobile-settings" type="button" :aria-current="workspace === 'settings' ? 'page' : undefined" aria-label="运行参数" @click="emit('select', 'settings')">
            <AppIcon name="settings" :size="16" />
          </button>
        </AppTooltip>
        <button class="button button-quiet topbar-refresh" type="button" :disabled="loading" :aria-busy="loading" aria-label="刷新当前数据" @click="emit('refresh')">
          <span v-if="loading" class="spinner" aria-hidden="true"></span>
          <AppIcon v-else name="refresh-cw" :size="14" />
          <span class="topbar-refresh-label">{{ loading ? "同步中" : "刷新" }}</span>
        </button>
    ...
    <div class="workspace-layout">
      <aside class="workspace-rail" aria-label="管理工作区">
        ...
        <button
          ...
          type="button"
          :class="{ active: workspace === 'settings' }"
          :aria-current="workspace === 'settings' ? 'page' : undefined"
          @click="emit('select', 'settings')"
        >
          <AppIcon name="settings" :size="16" /><span>运行参数</span>
        </button>
      </aside>
    ...
    <nav class="workspace-mobile-nav" aria-label="管理工作区">
      <button
        v-for="item in operationalItems"
        :key="item.id"
        type="button"
    ...
    <CommandPalette
      ...
      :theme="theme"
      @update:open="setCommandOpen"
      @select="emit('select', $event)"
      @refresh="emit('refresh')"
      @toggle-auto-refresh="emit('toggleAutoRefresh')"

...
const metrics = computed<Metric[]>(() => {
  ...
  return [
    ...
    },
    {
      ...
      hint: `活跃 ${snapshot.proxies.active} · 429 冷却 ${snapshot.egress.rateLimited} · 403 封禁 ${snapshot.egress.blocked} · 可铸票 ${snapshot.proxiesMintable}`,
      icon: "globe",
      tone: snapshot.egress.usable === 0 ? "bad" : snapshot.egress.rateLimited + snapshot.egress.blocked > 0 ? "warn" : "good",
    },
    {
      key: "minters",
      label: "在线授权服务",
...
const issues = computed(() => {
  ...
  if (snapshot.egress.usable === 0 && snapshot.proxies.active > 0) {
    list.push({ id: "allthrottled", title: "所有出口都在冷却中", detail: "上游按出口 IP 限流或封禁，当前无可用 IP。扩充代理数量是唯一有效的缓解手段。", tone: "bad" });
  } else {
    if (snapshot.egress.blocked > 0) {
      list.push({
        id: "blocked",
        title: `${snapshot.egress.blocked} 个出口被上游封禁`,
        detail: "这些 IP 遭 403，已按 403 冷却时长暂停。如果反复出现，说明这些 IP 已被上游标记，建议替换。",
        tone: "bad",
      });
    }
    if (snapshot.egress.rateLimited > 0) {
      list.push({
        id: "throttled",

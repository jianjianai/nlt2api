...
`preamble` 可选，输出频率由播报档位控制（管理后台可运行时调整，也可用 `NEURALWATT_PREAMBLE_VERBOSITY` 设定环境默认）：`normal`（默认）要求模型用一句话播报每个非平凡步骤——正在做什么或刚发现了什么，仅在上一步已隐含时省略；`verbose` 要求每步都播报；`quiet` 恢复旧行为，只在关键节点（做出决定、找到根因、阶段变化、风险操作）输出。无论哪档，`preamble` 都应具体、有信息量，而不是笼统的“我要开始…”，且不得超过一句话。契约在 normal/verbose 档会把 `preamble` 直接展示在信封骨架里（shown, not described），因为只靠在散文里描述“可以输出 preamble”时，模型会过度服从省略默认而全程静默。适配器会在完整信封校验通过后释放 `preamble`。

...
<template>
  <section class="workspace-page accounts-workspace">
    <header class="page-heading page-heading-actions">
      ...
      <button class="button button-primary" type="button" @click="emit('add')">添加账号</button>
    </header>

    <div class="workspace-toolbar">
      <label class="search-field"><span class="sr-only">搜索账号</span><input :value="query" type="search" placeholder="搜索名称、邮箱或代理" @input="emit('update:query', ($event.target as HTMLInputElement).value)" /></label>
      <span class="toolbar-summary">{{ filtered.length }} / {{ accounts.length }} 个账号</span>
    </div>

    <div v-if="accounts.length === 0" class="workspace-empty"><strong>尚未配置账号</strong><p>添加门户账号后，网关才能接收模型请求。</p><button class="button button-primary" type="button" @click="emit('add')">添加第一个账号</button></div>
    ...
    <div v-else class="account-compact-grid">
      <article v-for="account in filtered" :id="`account-${account.id}`" :key="account.id" class="account-compact-card" :class="{ disabled: !account.enabled, expanded: expandedId === account.id }">
        ...
        <div class="account-meter" aria-hidden="true"><i :style="{ width: `${Math.min(100, account.runtime.requestsLastMinute / (account.schedulerOverrides?.accountRpm ?? scheduler.accountRpm) * 100)}%` }"></i></div>
        ...
        <footer class="account-card-footer"><button class="button button-quiet" type="button" :aria-expanded="expandedId === account.id" @click="toggleExpanded(account)">{{ expandedId === account.id ? "收起" : "管理" }}</button></footer>

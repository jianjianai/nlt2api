...
<template>
  <section class="workspace-page records-workspace">
    <header class="page-heading page-heading-actions"><div><p class="section-kicker">请求诊断</p><h1>请求记录</h1><p>按客户端请求查看首次调用、重试、纠错和续写的完整链路。</p></div><div class="page-actions"><label class="recording-control"><span>{{ recording ? "记录已开启" : "记录已关闭" }}</span><button class="switch" :class="{ on: recording }" type="button" :aria-pressed="recording" @click="emit('toggleRecording', !recording)"><span></span></button></label><button class="button button-quiet" type="button" @click="emit('refresh')">刷新</button><details class="overflow-menu"><summary aria-label="更多记录操作">更多</summary><button class="danger" type="button" :disabled="!records.length || clearing" @click="emit('clear')">{{ clearing ? "清空中…" : "清空全部记录" }}</button></details></div></header>
    <div class="records-toolbar"><label class="search-field"><span class="sr-only">搜索请求记录</span><input :value="query" type="search" placeholder="搜索端点或账号" @input="emit('update:query', ($event.target as HTMLInputElement).value)" /></label><div class="segmented-control" role="group" aria-label="记录状态筛选"><button type="button" :class="{ active: filter === 'all' }" @click="emit('update:filter', 'all')">全部 {{ records.length }}</button><button type="button" :class="{ active: filter === 'success' }" @click="emit('update:filter', 'success')">成功 {{ records.length - failedCount }}</button><button type="button" :class="{ active: filter === 'failed' }" @click="emit('update:filter', 'failed')">失败 {{ failedCount }}</button></div></div>
    ...
    <div v-else class="conversation-workbench">
      ...
      </section><section v-else-if="loadingDetail" class="conversation-detail"><p class="parsed-empty">正在加载记录详情…</p></section>
    </div>
  </section>
</template>

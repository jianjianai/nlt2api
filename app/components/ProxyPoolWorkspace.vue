...
import type { ProxyImportLineResult, ProxyPoolEntry, ProxyPoolSettings, ProxyPoolStatus, ProxySyncRun, ProxySyncSettings } from "../types/admin.ts";
import { proxyPolicySummary } from "../utils/admin-ui.ts";

const props = defineProps<{
  proxies: ProxyPoolEntry[];
  draft: ProxyPoolSettings;
  syncDraft: ProxySyncSettings;
  syncRun: ProxySyncRun | null;
  importText: string;
...
const emit = defineEmits<{
  ...
  updateSync: [field: keyof ProxySyncSettings, value: boolean | number];
  import: [];
...
const counts = computed(() => ({
  ...
  untested: props.proxies.filter((proxy) => proxy.status === "untested").length,
...
const statusLabel: Record<ProxyPoolStatus, string> = { untested: "未测活", idle: "空闲", in_use: "使用中", error: "错误", checking: "检测中" };
const outcomeLabel = { created: "已新增", existing: "已存在", invalid: "格式错误" } as const;

function setSync(field: keyof ProxySyncSettings, event: Event): void {
  const target = event.target as HTMLInputElement;
  emit("updateSync", field, target.type === "number" ? Number(target.value) : target.checked);
}
...
<template>
  <section class="workspace-page proxy-workspace">
    ...
    <section class="content-section"><div class="section-heading"><div><h2>免费代理同步</h2><p>从 Rola 抓取公网 IPv4，完成 DeepInfra 目录与最小 Chat 测活后，原账号保持 ID 并自动换绑出口。</p></div><button class="button button-primary" type="button" :disabled="runningSync" :aria-busy="runningSync" @click="emit('runSync')"><span v-if="runningSync" class="spinner" aria-hidden="true"></span><AppIcon v-else name="refresh-cw" :size="14" />{{ runningSync ? "同步中" : "立即同步" }}</button></div><div class="policy-grid"><label class="toggle-field"><span><strong>周期同步</strong><small>第一个周期结束后自动运行，不在服务启动时立即抓取。</small></span><AppSwitch :model-value="syncDraft.enabled" label="周期同步" @update:model-value="emit('updateSync', 'enabled', $event)" /></label></div><div class="field-grid"><label><span>目标代理账号</span><input :value="syncDraft.targetAccountCount" type="number" min="0" max="500" @input="setSync('targetAccountCount', $event)" /></label><label><span>周期（分钟）</span><input :value="syncDraft.intervalMinutes" type="number" min="5" max="1440" @input="setSync('intervalMinutes', $event)" /></label><label><span>候选上限</span><input :value="syncDraft.candidateLimit" type="number" min="1" max="2000" @input="setSync('candidateLimit', $event)" /></label><label><span>测活并发</span><input :value="syncDraft.probeConcurrency" type="number" min="1" max="100" @input="setSync('probeConcurrency', $event)" /></label><label><span>测活超时（秒）</span><input :value="syncDraft.probeTimeoutSeconds" type="number" min="1" max="120" @input="setSync('probeTimeoutSeconds', $event)" /></label><label><span>失败阈值</span><input :value="syncDraft.failureThreshold" type="number" min="1" max="20" @input="setSync('failureThreshold', $event)" /></label></div><p class="field-hint">修改后自动保存；「立即同步」会先确认当前设置已写入服务端。</p><p v-if="syncRun" class="field-hint">最近批次 {{ syncRun.status }} · 抓取 {{ syncRun.counts.fetched }} · 健康 {{ syncRun.counts.healthy }} · 替换 {{ syncRun.counts.replaced }} · 新增 {{ syncRun.counts.created }}</p><div class="form-actions"><button class="button button-quiet" type="button" :disabled="savingSync" :aria-busy="savingSync" @click="emit('saveSync')"><span v-if="savingSync" class="spinner" aria-hidden="true"></span><AppIcon v-else name="check" :size="14" />{{ savingSync ? "保存中" : "保存同步设置" }}</button></div></section>

    <div class="resource-summary" aria-label="代理池状态"><button v-for="item in [{ id: 'all', label: '全部' }, { id: 'untested', label: '未测活' }, { id: 'idle', label: '空闲' }, { id: 'in_use', label: '使用中' }, { id: 'error', label: '错误' }, { id: 'checking', label: '检测中' }]" :key="item.id" type="button" :class="{ active: filter === item.id }" :aria-pressed="filter === item.id" @click="emit('update:filter', item.id as typeof filter)"><span>{{ item.label }}</span><strong>{{ counts[item.id as keyof typeof counts] }}</strong></button></div>

...
<template>
  <section class="workspace-page scheduler-workspace">
    ...
    <section class="content-section"><div class="section-heading"><div><h2>出口运行状态</h2><p>当前出口的账号数、RPM 和下一可用时间。</p></div></div><div v-if="runtime.egresses.length" class="egress-runtime-table"><div class="egress-runtime-head" aria-hidden="true"><span>出口 ID</span><span>账号</span><span>请求 / RPM</span><span>限流</span><span>下一可用</span></div><div v-for="egress in runtime.egresses" :key="egress.id" class="egress-runtime-row"><code>{{ egress.id }}</code><span data-label="账号">{{ egress.accountCount }}</span><span data-label="请求 / RPM">{{ egress.requestsLastMinute }} / {{ egress.rpm }}</span><span data-label="限流"><i class="table-status" :class="{ active: egress.limited }" aria-hidden="true"></i>{{ egress.limited ? "已启用" : "未启用" }}</span><span data-label="下一可用">{{ egress.nextRateAvailableAt ? new Date(egress.nextRateAvailableAt).toLocaleTimeString() : "当前可用" }}</span></div></div><div v-else class="mini-empty">暂无出口运行数据。</div></section>
  </section>
</template>

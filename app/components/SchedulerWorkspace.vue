...
<template>
  <section class="workspace-page scheduler-workspace">
    <header class="page-heading page-heading-actions"><div><p class="section-kicker">容量与排队</p><h1>调度策略</h1><p>控制账号、模型、出口和等待队列的统一准入容量。</p></div><button class="button button-primary" type="button" :disabled="saving" @click="emit('save')">{{ saving ? "保存中…" : "保存调度策略" }}</button></header>
    <div class="runtime-strip"><div><span>排队请求</span><strong>{{ runtime.pending }}</strong></div><div><span>最老等待</span><strong>{{ Math.ceil(runtime.oldestWaitMs / 1000) }} 秒</strong></div><div><span>出口组</span><strong>{{ runtime.egresses.length }}</strong></div></div>
    ...
    <section class="content-section"><div class="section-heading"><div><h2>出口运行状态</h2><p>当前出口的账号数、RPM 和下一可用时间。</p></div></div><div v-if="runtime.egresses.length" class="egress-runtime-table"><div class="egress-runtime-head"><span>出口 ID</span><span>账号</span><span>请求 / RPM</span><span>限流</span><span>下一可用</span></div><div v-for="egress in runtime.egresses" :key="egress.id" class="egress-runtime-row"><code>{{ egress.id }}</code><span>{{ egress.accountCount }}</span><span>{{ egress.requestsLastMinute }} / {{ egress.rpm }}</span><span>{{ egress.limited ? "已启用" : "未启用" }}</span><span>{{ egress.nextRateAvailableAt ? new Date(egress.nextRateAvailableAt).toLocaleTimeString() : "当前可用" }}</span></div></div><div v-else class="mini-empty">暂无出口运行数据。</div></section>
  </section>
</template>

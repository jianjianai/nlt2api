...
管理面板位于 `http://localhost:3000/`（也可能使用 Vite 自动选择的端口）。面板会要求输入管理员令牌，令牌只保存在当前浏览器会话中。登录后使用六个职责明确的工作区：运行概览、账号、代理池、调度策略、请求记录和网关设置。桌面端使用左侧工作台，移动端使用底部导航；首次进入显示概览，之后记住上次工作区。面板提供浅色和暗色两种主题，旧的高级灰选择会自动迁移到暗色，主题只保存在当前浏览器本地。账号工作区支持服务端分页、搜索、状态和分组筛选；账号可属于多个分组，每个分组可签发仅访问该组账号的客户端 API Key。账号密码和客户端 API Key 默认遮罩，可短时显示或直接复制；分组 API Key 明文只在创建或轮换时显示一次。切换工作区、窗口失焦或退出登录会重新遮罩敏感值。
...
- `GET /api/admin/analytics`：按最多 366 天的时间范围返回有界趋势和单模型消费聚合
- `POST /api/admin/analytics/prices/refresh`：手动刷新门户价格目录
- `PATCH /api/admin/analytics/retention`：配置执行明细和分钟桶保留天数；`null` 表示永久保留
- `POST /api/admin/analytics/cleanup/preview`、`POST /api/admin/analytics/cleanup`：先预览再使用一次性 token 确认清理
...
「运行概览」展示客户端 RPM、真实上游 RPM、调用放大率、60 分钟趋势、容量使用率、按模型消费和扩容建议。扩容算法结合 5/15/60 分钟 EWMA、20% 安全余量、P95 上游服务时长、P95 调用放大率、账号共享 RPM、账号/模型并发和共享出口 RPM，并使用联合容量分配避免多模型重复计算同一账号。共享出口先饱和时只建议新增独立出口，不误报新增账号。样本不足时明确显示低置信度，不给出强账号数。

精确用量账本从功能首次运行时开始，不回填旧调试记录。所有初始、重试、工具纠错和思考续写的实际上游 token 都进入同一幂等执行账本；提示词、回复正文、Cookie、密码和完整代理 URL 不进入分析数据库。费用是按厂商精确 API ID 官方价格或 NeuralWatt 精确门户模型 ID 目录价计算的 API 等价估算，不等同于门户套餐、赠送额度、退款或实际发票。未定价请求仍统计 token，但不会静默按 0 计入“完整消费”。历史执行冻结不可变价格版本，后续目录刷新不会重算旧账。

## 限制与运维

分析账本保存在 `NEURALWATT_DATA_DIR/usage-analytics.sqlite`，使用 Node 22 自带的 `node:sqlite`、SQLite WAL、外键和 schema migration。生产运行要求项目声明的 Node 22（`>=22.12 <23`）；部署前应确认数据目录可写，并将整个 `NEURALWATT_DATA_DIR` 与当前原子发布一同备份。应用代码回滚不得删除或降级该数据库，旧版本会忽略它。分析写入和调度分钟桶在请求响应之外执行；数据库、价格目录或预测降级不会把成功的模型请求改成失败，并会通过补偿队列重试。

分析明细和分钟桶默认永久保留；「网关设置」可分别设置按天保留，也可按截止时间预览并二次确认手工清理。日/月模型总账、价格版本和清理审计永久保留，清理前后会校验聚合不变。定时维护每小时执行保留、补偿和价格检查；门户目录启动时及每 24 小时刷新，失败时沿用最后有效版本并显示陈旧/错误状态。

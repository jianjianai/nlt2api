...
`GET /v1/models` 和 `GET /v1/models/:id` 返回当前门户公开模型目录。无工具轮在上游每个 SSE 分片到达时立即转换并转发：Chat 会分别发送 `reasoning`、`reasoning_content` 和 `content` delta。流式链路带读取背压；客户端断开会立即取消上游请求并释放账号，损坏的上游 SSE 会失败关闭。上游退回 JSON 时会自动降级为一次性 SSE，不会使客户端等待或中断。带工具的请求会有意缓冲上游响应，校验服务端控制的工具调用信封，然后合成为标准 OpenAI `tool_calls`（可选携带用户可见的 `content`/preamble）；这是为了避免执行半截或无效的工具调用。信封同时支持 JSON 与 XML 两种线格式：契约会并列展示两种骨架，由上游模型自选更擅长的格式（不同模型对 JSON/XML 的敏感度不同），也可用 `NEURALWATT_TOOL_CALL_FORMAT=json|xml` 固定其中一种；无论契约展示哪种，解析端始终同时接受两种格式。两种格式都带自修复（JSON 用 jsonrepair；XML 采用 fast-xml-parser 严格校验定位 + txml 宽松解析兜底，可恢复截断标签、裸 `&`、Markdown 围栏/前后散文、裸 `<tool_call>` 片段等 LLM 常见错误），修复失败时纠错提示会带出行列号、附近文本摘录和格式切换建议。XML 参数值按声明的 JSON Schema 定向转型（数字/布尔不带引号，数组/对象可写 JSON 文本），随后与 JSON 路径共用同一条 Ajv 校验流水线。网关不会执行客户端提供的工具；客户端应在下一轮发送 `role: "tool"`。工具轮的 SSE 在校验完成后生成，因此不能降低该轮首 token 延迟。
